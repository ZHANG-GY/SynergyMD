# -*- coding: utf-8 -*-
"""
Created on Thu Dec 18 16:25:07 2025
...
sample = np.load('new sample42626.npy')
skf1 = KFold(n_splits=5,shuffle=True)
mir_gcn_fea = np.load("mir_gcn3.npy")
mir_seq_fea = np.load("human_miRNA_fea.npy")
mir_seq_fea = transfer.fit_transform(mir_seq_fea)
with open("human_miRNA_name1917.pkl",'rb') as f:
    mir_name = pickle.load(f)

with open("miRNA_name_gae_1602.pkl",'rb') as f:
    miRNA_name_1602 = pickle.load(f)
mir_gcn_fea = mir_gcn_fea[:1602]

dis_gcn_fea = np.load("dis_gcn3.npy")
dis_gcn_fea = dis_gcn_fea[:7732]
dis_gcn_fea = transfer.fit_transform(dis_gcn_fea)

with open("mesh_7732.pkl",'rb') as f:
    mesh_name_7732 = pickle.load(f)
with open("mesh_name_gae_7053.pkl",'rb') as f:
    mesh_name_7053 = pickle.load(f)
dis_txt_fea = np.load("dis_defn_fea7053.npy")
...
sample_mir = list(set([i[0] for i in sample]))
sample_dis = list(set([i[1] for i in sample]))
...
for time in range(2,12):
    train_sample,test_sample,train_label,test_label = train_test_split(sample,label,test_size=0.1,random_state=time,stratify=label)
    ...
    MLP_fenlei = MLP2(512,256,64,1,0.8).to(device)
    optimizer_MLP_fenlei = torch.optim.Adam(MLP_fenlei.parameters(),lr=0.0002,weight_decay=0.001)
    ...
    t_mir_idx = [miRNA_name_1602.index(i[0]) for i in train_sample] 
    t_dis_idx = [mesh_name_7053.index(i[1]) for i in train_sample]
    t_sample_fea = torch.concatenate((mir_seq_fea1[t_mir_idx],dis_txt_fea1[t_dis_idx]),axis=1).to(device)
    train_data = TensorDataset(t_sample_fea,train_label)
    train_loader = DataLoader(train_data, batch_size=1024, shuffle=False)
    test_mir_idx = [miRNA_name_1602.index(i[0]) for i in test_sample] 
    test_dis_idx = [mesh_name_7053.index(i[1]) for i in test_sample]
    
    test_sample_fea = torch.concatenate((mir_seq_fea1[test_mir_idx],dis_txt_fea1[test_dis_idx]),axis=1).to(device)
